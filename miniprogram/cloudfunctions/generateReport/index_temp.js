  ...
  try {
    ...
    */
    ...
    try {
      ...
      // 4. 调用 generatePDF 云函数生成 PDF
      console.log('[Cloud] 调用 generatePDF 云函数生成 PDF...');
      const pdfRes = await callWithTimeout(
        () => cloud.callFunction({
          name: 'generatePDF',
          data: { answerId: reportId }
        }),
        55000
      );
      if (!pdfRes || !pdfRes.result || !pdfRes.result.fileID) {
        throw new Error('generatePDF 未返回 fileID: ' + JSON.stringify(pdfRes));
      }

      // 回写 fileID 到数据库
      uploadRes = { fileID: pdfRes.result.fileID };

      await db.collection('assessments').doc(reportId).update({
        data: { fileID: uploadRes.fileID, updatedAt: db.serverDate() }
      });
      console.log('[Cloud] PDF fileID 回写成功：', pdfRes.result.fileID);

      /*      // 调用云托管 pdf-service 生成 PDF
      if (reportId) {
        try {
          const pdfRes = await callWithTimeout(
            () => cloud.callContainer({
              ...
              data: { answerId: reportId, pdfNo: pdfNo }
            }),
            55000
          );
          const pdfResult = pdfRes && pdfRes.data ? pdfRes.data : null;
          console.log('[Cloud] pdf-service 返回：', pdfResult);

          if (pdfResult && pdfResult.success && pdfResult.fileID) {
            await db.collection('assessments').doc(reportId).update({
              data: { fileID: pdfResult.fileID, updatedAt: db.serverDate() }
            });
            console.log('[Cloud] fileID 回写成功：', pdfResult.fileID);
          } else {
            console.error('[Cloud] pdf-service 未返回 fileID：', pdfResult);
          }
        } catch (pdfErr) {
          console.error('[Cloud] pdf-service 调用失败（非阻断）：', pdfErr.message);
        }
      } */
    } catch (dbErr) {
      console.error('[Cloud] try块内部失败:', dbErr.message, dbErr.stack);
    }
