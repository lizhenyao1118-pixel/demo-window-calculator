...
const { resolveGlassConfig } = require('./shared/arbitrator.js');
const { getClimateZone, CLIMATE_SPEC } = require('./shared/climateSpec.js');
const { calcSealGrades } = require('./shared/calculator-v2.js');
...
function buildUpgradeReasons(answers) {
  ...
  if (floor > 16 || floor / totalFloors > 0.6) {
    reasons.push(`第${floor}层高层风压要求（≥4.07kPa），A档型材壁厚余量有限`);
  }
  if (TRAFFIC_NOISE_TYPES.includes(answers.noise_type)) {
    reasons.push(`轨道/交通噪声场景隔声目标Rw≥40dB，A档玻璃配置工程余量有限`);
  }
  return reasons;
}

function calcCostDelta(fromTier, toTier, answers) {
  const fromSpec = fromTier === 'A' ? BUDGET_SPEC['A'] : BUDGET_SPEC[fromTier.toUpperCase()];
  const toSpec = toTier === 'A' ? BUDGET_SPEC['A'] : BUDGET_SPEC[toTier.toUpperCase()];

  return {
    amount: toSpec.price_range.split('–')[1] - fromSpec.price_range.split('–')[1],
    unit: '元/㎡',
    note: `相对于本案${fromTier.toUpperCase()}档实际配置（夹胶中空）`
  };
}
