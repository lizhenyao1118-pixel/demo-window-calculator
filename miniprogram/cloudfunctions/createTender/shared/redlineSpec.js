function buildRedlineRegistry({ TERM, getTierLabel, getField }) {
  return [
    ...
    { id: 'R02', text: (a, r) => `型材系统：主受力壁厚≥1.8mm（当前风压要求P3≥${getField(r, 'P3')}kPa），须提供截面检测报告`, level: 'mandatory', softened: false, trigger: () => true, userMeaning: '壁厚是型材抗风压的主要参数之一。商家常见说法是"我们产品质量很好"，但不提供壁厚数据。壁厚不足在极端天气下可能导致型材变形或破坏，须要求提供截面检测报告。' },
    ...
    // 抗风主线
    { id: 'R13', text: '落地窗、大面积玻璃须使用安全玻璃（钢化或夹胶）；超大尺寸/面积场景需加厚或采用夹胶构造以满足强度要求，推荐夹胶（同时覆盖安全与强度两项要求）', level: 'mandatory', softened: false, trigger: (a, r) => !!(r && r.needsSafetyGlass), group: '安全玻璃', userMeaning: '本案存在高碰撞风险部位（落地窗/儿童活动区），此类场景应采用夹层安全玻璃，碎片不脱落。商家若以"钢化玻璃同样安全"替代，须提供该部位适用的安全玻璃类型依据，否则不予接受。' },

    // 安全主线
    { id: 'R14', text: '须提供安全玻璃 3C 认证标识或型号可查文件', level: 'mandatory', softened: false, trigger: (a, r) => !!(r && r.needsSafetyGlass), userMeaning: '本案存在高碰撞风险部位（落地窗/儿童活动区），此类场景应采用夹层安全玻璃，碎片不脱落。商家若以"钢化玻璃同样安全"替代，须提供该部位适用的安全玻璃类型依据，否则不予接受。' },
